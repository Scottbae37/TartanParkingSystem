...
// call onCancel() on ESCAPE
    ...
    public PaymentDialog(Reservation rsvp) {
        this();
        mRsvp = rsvp;
    }
    ...
    private void onOK() {
        ...
        tmpPayment.setCcNum(ccNumTextField.getText());
        tmpPayment.setCcExpDate(expDate);
        tmpPayment.setCcName(nameTextField.getText());

        if (validatePaymentInfo(tmpPayment)) {
            payment = tmpPayment;
            dispose();
        }
    }

    private boolean validatePaymentInfo(Payment p) {
        boolean is = true;
        String msg = null;
        ...
        if (TartanUtils.IS_EMPTY.test(ccNum)) {
            ...
            is = false;
            msg = "Invalid name\nIt's different from the card owner's name.\n check it now!";
        }else if (p.isValid() == false) {
            is = false;
            msg = "Invalid date, check it now!";
        }
